...
const DB = {
    ...
    init: () => { const s=localStorage.getItem('sys_v11'); if(s) DB.data=JSON.parse(s); else DB.seed(); },
    save: () => { localStorage.setItem('sys_v11', JSON.stringify(DB.data)); App.updateDash(); },
    seed: () => { DB.data.sites=[{id:1,name:'Matriz SP'}]; DB.data.types=[{id:10,name:'Câmera IP',isVideo:true}]; DB.save(); },
    reset: () => { if(confirm('RESETAR TUDO?')){ localStorage.removeItem('sys_v11'); location.reload(); } },
...
const App = {
    ...
    updateDash: () => {
        ...
        // 2. O.S. Abertas
        const osCount = DB.data.os.filter(o => o.status === 'aberto').length;

        // 3. Saúde das Câmeras (Lógica: Total Câmeras - Câmeras com OS Aberta)
        let totalCams = 0;
        let brokenCams = 0;
        
        // Pega IDs de tipos que são vídeo
        const vidTypeIds = DB.data.types.filter(t => t.isVideo).map(t => t.id);
        
        // Pega equipamentos que são desses tipos
        const allCams = DB.data.equips.filter(e => {
            // Acha o grupo do equip para saber o tipo
        ...
        if(totalCams > 0) {
            // Verifica quantas dessas câmeras tem OS aberta
        ...
        if(document.getElementById('dash-ro')) {
            document.getElementById('dash-ro').innerText = roCount;
            document.getElementById('dash-os').innerText = osCount;
            document.getElementById('dash-health').innerText = health + '%';
            
            // Muda cor da saúde se estiver baixa
            const cardHealth = document.getElementById('dash-health').parentElement;
            if(health < 90) cardHealth.style.background = 'rgba(239, 68, 68, 0.85)'; // Vermelho
            else cardHealth.style.background = 'rgba(16, 185, 129, 0.85)'; // Verde
        }
    }
};
...
const OS = {
    ...
    render: () => {
        ...
        openContainer.innerHTML = ''; doneContainer.innerHTML = '';
        if(list.length === 0) openContainer.innerHTML = '<p style="padding:20px; color:#888; text-align:center">Nenhuma O.S.</p>';
        ...
        list.forEach(os => {
            ...
            const btnFinish = os.status === 'aberto' ? `<button class="btn btn-sm btn-success" title="Concluir" onclick="OS.finish(${os.id})"><i class="fas fa-check"></i></button>` : `<span style="font-size:11px; color:#10b981; margin-right:5px"><i class="fas fa-check-double"></i> Fechado em ${os.dateClose}</span>`;
            const html = `<div class="os-card p-${os.priority}"><div class="os-header"><span>#${os.id} • ${os.dateOpen}</span><span>${site.name}</span></div><div class="os-title">${group.name} - ${itemName}</div><div class="os-desc">${os.desc}</div><div style="display:flex; justify-content:space-between; align-items:center; margin-top:10px; border-top:1px solid #eee; padding-top:10px"><div><button class="btn btn-sm btn-dark" title="Imprimir" onclick="OS.printTicket(${os.id})"><i class="fas fa-print"></i></button><button class="btn btn-sm btn-danger" title="Excluir" onclick="OS.del(${os.id})"><i class="fas fa-trash"></i></button></div><div>${btnFinish}</div></div></div>`;
            if(os.status === 'aberto') openContainer.innerHTML += html; else doneContainer.innerHTML += html;
        });
    },
    prompt: (equipId) => { document.getElementById('os-input-equip-id').value = equipId; document.getElementById('os-input-desc').value = ''; document.getElementById('modal-os').style.display = 'flex'; },
    ...
    create: () => {
        ...
        DB.data.os.push({ id: Date.now(), equipId: equipId, siteId: equip?equip.siteId:0, status: 'aberto', priority: prio, desc: desc, dateOpen: new Date().toLocaleDateString('pt-BR'), dateClose: null });
        DB.save(); OS.closeModal(); Toast.show("O.S. Criada!");
    },
    finish: (id) => { if(confirm("Confirmar conclusão?")) { const os = DB.data.os.find(o => o.id === id); if(os) { os.status = 'concluido'; os.dateClose = new Date().toLocaleDateString('pt-BR'); DB.save(); OS.render(); } } },
    del: (id) => { if(confirm("Deseja apagar esta O.S.?")) { DB.data.os = DB.data.os.filter(o => o.id !== id); DB.save(); OS.render(); Toast.show("O.S. Excluída"); } },
    printTicket: (id) => {
        const os = DB.data.os.find(o => o.id === id); if(!os) return;
        const equip = DB.data.equips.find(e => e.id === os.equipId);
        const site = equip ? DB.data.sites.find(s => s.id === equip.siteId) : {name:'?'};
        const win = window.open('', '', 'width=400,height=600');
        win.document.write(`<html><head><title>O.S. #${os.id}</title><style>body{font-family:'Courier New',monospace;padding:20px}.box{border:2px solid #000;padding:15px}h2{text-align:center;border-bottom:2px dashed #000;padding-bottom:10px}.label{font-weight:bold;margin-top:10px;display:block}.val{margin-bottom:10px;display:block}</style></head><body><div class="box"><h2>ORDEM DE SERVIÇO</h2><span class="label">ID / DATA:</span><span class="val">#${os.id} - ${os.dateOpen}</span><span class="label">LOCAL:</span><span class="val">${site.name}</span><span class="label">EQUIPAMENTO:</span><span class="val">${equip.name}</span><span class="label">PRIORIDADE:</span><span class="val" style="text-transform:uppercase">${os.priority}</span><div style="border:1px dashed #000;margin:15px 0"></div><span class="label">DEFEITO RELATADO:</span><span class="val">${os.desc}</span><div style="margin-top:40px;text-align:center;border-top:1px solid #000;padding-top:10px">Assinatura Técnico</div></div><script>window.print();</script></body></html>`);
        win.document.close();
    }
};
...
const Occurrences = {
    init: () => {
        ...
        document.getElementById('ro-site').innerHTML = DB.data.sites.map(s=>`<option value="${s.id}">${s.name}</option>`).join('');
    ...
    showTab: (tab) => {
        document.getElementById('occ-tab-form').style.display = tab==='form'?'block':'none';
        document.getElementById('occ-tab-open').style.display = tab==='open'?'block':'none';
        document.getElementById('occ-tab-closed').style.display = tab==='closed'?'block':'none';
        const btns = document.querySelectorAll('.tab-btn');
        btns.forEach(b => b.classList.remove('active'));
        if(tab==='form') btns[0].classList.add('active');
        if(tab==='open') { btns[1].classList.add('active'); Occurrences.renderLists(); }
        if(tab==='closed') { btns[2].classList.add('active'); Occurrences.renderLists(); }
    },
    toggleCamField: () => {
    ...
    resetForm: () => {
        ...
        Occurrences.toggleCamField(); Occurrences.toggleAreaField();
        const now = new Date(); now.setMinutes(now.getMinutes() - now.getTimezoneOffset());
        document.getElementById('ro-date').value = now.toISOString().slice(0,10);
        document.getElementById('ro-time').value = now.toTimeString().slice(0,5);
        document.getElementById('occ-display-id').innerText = "ID: RO-" + new Date().getFullYear() + "-" + (DB.data.occurrences.length + 1000);
    ...
    save: () => {
        ...
        const finalizeSave = (pData) => {
            ...
            DB.save(); Toast.show("R.O. Salvo! Formulário limpo."); Occurrences.resetForm();
        };
    ...
    confirmFinish: () => {
        ...
        if(item) {
            ...
            DB.save(); document.getElementById('modal-finish-ro').style.display = 'none'; Toast.show("R.O. Finalizado!"); Occurrences.renderLists();
        }
    },
    printRO: (id) => {
        const item = DB.data.occurrences.find(x => x.id === id); if(!item) return;
    ...
    renderLists: () => {
        ...
        openDiv.innerHTML = list.filter(x => x.status === 'Aberto').map(item => {
            return `<div class="card" style="border-top:4px solid #f59e0b"><div style="display:flex; justify-content:space-between; margin-bottom:10px"><b>${item.roNumber}</b><span class="badge b-open">ABERTO</span></div><div style="font-size:12px; color:#666; margin-bottom:10px"><b>${item.type}</b> <br> ${item.date} às ${item.time}</div><p style="font-size:13px; margin-bottom:15px; background:#f9f9f9; padding:5px; height:60px; overflow:hidden">${item.desc.substring(0,100)}...</p><div style="display:flex; gap:5px"><button class="btn btn-sm btn-dark" style="flex:1" onclick="Occurrences.printRO(${item.id})"><i class="fas fa-file-pdf"></i> PDF</button><button class="btn btn-sm btn-success" style="flex:1" onclick="Occurrences.openFinishModal(${item.id})">FINALIZAR</button></div></div>`;
        }).join('') || '<p style="color:#888">Nenhuma pendente.</p>';
        closedDiv.innerHTML = list.filter(x => x.status === 'Finalizado').map(item => {
            return `<div class="list-item"><div><b>${item.roNumber}</b> - ${item.type}<div style="font-size:11px; color:#888">Encerrado em: ${item.endDate}</div></div><div><button class="btn btn-sm btn-dark" onclick="Occurrences.printRO(${item.id})"><i class="fas fa-file-pdf"></i> PDF / VER</button></div></div>`;
...
const Logic = {
    saveCheck: (id, st, type) => {
        ...
        DB.save(); loadFn(); Toast.show('Salvo');
        if(st === 'Erro') {
            const existingOS = DB.data.os.find(o => o.equipId === id && o.status === 'aberto');
            if(!existingOS) OS.prompt(id); else Toast.show("Já existe O.S. aberta.");
        }
    }
};
